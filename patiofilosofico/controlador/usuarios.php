                ...
                <div class="row">
                    <div class="col-9 offset-sm-0 panel-body">
                        ...
                        <div class="col-sm-12">
                            ...
                            if($user->getId()>0){
                            ?>
                            <button data-toggle="modal" data-target="#exampleModal" onclick="$('#modal_cuerpo_perfiles').html('');cargarPagina('perfil_Usuario.php?idUser=<?php echo $r->getId(); ?>', 'modal_cuerpo_perfiles', true)" class="btn btn-sm btn-primary">Ver perfil</button>
                            <?php
                            }
                            ?>
                        </div>
...
//            print_r($r);
    ...
    if ($p['oper'] == "invitar a grupo") {
        ...
        $idSeguir = $p["idseguir"];
//        echo $idSeguir;
        if($user->seguiraUsuario($idSeguir)){
        ?>
        <script>
        ...
        <?php
            ...
            if (typeof noty !== 'undefined')
                noty.dismiss();
            noty = new NotificationFx({
                message: '<p> Ha ocurrido un error al intentar agregar el contacto, por favor intentalo mas tarde... </p>',
                layout: 'growl',
                effect: 'slide',
    ...
    if ($p['oper'] == "desactivar Usuario") {
        $idUser = $p["idusuario"];
//        $MIRESP = $user->dejarSeguiraUsuario($idSeguir);
        $user->actualizarEstado($idUser, 'BLOQUEADO')
        ?>
        <script>
            ...
            if (typeof noty !== 'undefined')
                noty.dismiss();
            noty = new NotificationFx({
                message: '<b>Usuario desactivado</b><br><p>El usuario ya no puede acceder a la plataforma...</p>',
                layout: 'growl',
                effect: 'slide',
                type: 'warning' // notice, warning or error
            });
        //            noty.dismiss();
            noty.show();
                    ...
                    cargarPagina('admin_usuario.php', 'contenedorPrincipal', true);
        </script>
        <?php
    } ELSE
    if ($p['oper'] == "activar Usuario") {
        $idUser = $p["idusuario"];
//        $MIRESP = $user->dejarSeguiraUsuario($idSeguir);
        $user->actualizarEstado($idUser, 'ACTIVO')
        ?>
        <script>
            ...
            if (typeof noty !== 'undefined')
                noty.dismiss();
            noty = new NotificationFx({
                message: '<b> Usuario activado</b><br><p>El usuario puede ingresar en la plataforma y usar nuestros servicios...</p>',
                layout: 'growl',
                effect: 'slide',
                type: 'notice' // notice, warning or error
            });
