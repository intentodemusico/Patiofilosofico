...
FROM `mensaje` WHERE `idmensaje` = '" . $idMensaje . "' ;";
    ...
    static public function buscarMensajesSinLeer($idrecibe) {
        $sql = "SELECT `mensaje`.`idmensaje`,
    `mensaje`.`idenvia`,
    `mensaje`.`idrecibe`,
    `mensaje`.`mensaje`,
    `mensaje`.`fecha`,
    `mensaje`.`estado`
FROM `mensaje` where idrecibe= '".$idrecibe."' and estado = 'ENVIADO';";
        $conn = conBD::conectar();
        $resultado = mysqli_query($conn, $sql);
        $numeroChats = $resultado->num_rows;
//        $numeroChats = count($resultado);
//        echo 'numero de chat= '.$numeroChats;
//        $fila = mysqli_fetch_array($resultado, MYSQLI_ASSOC);
        mysqli_close($conn);
        return $numeroChats;
    }
