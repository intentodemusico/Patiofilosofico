...
//        print_r($resp);
    ...
    function buscarNuevosMensajesChatPorIdUser($iduser, $idRecibe) {
        $sql = "SELECT `mensaje`.`idmensaje`," .
                ...
                "`mensaje`.`fecha`," .
                "`mensaje`.`estado`," .
                "(SELECT `usuario`.`avatar` FROM `usuario` where idusuario = `mensaje`.idenvia) as avatar" .
                " FROM `mensaje`  WHERE (`idrecibe`='" . $idRecibe . "' OR `idenvia` = '" . $idRecibe . "') AND (`idrecibe`='" . $iduser . "' OR `idenvia` = '" . $iduser . "') AND `idenvia` <> '" . $iduser . "'  AND `estado` = 'ENVIADO'   order by fecha DESC ";
...
";
    ...
    function buscarSalasChatDisponibleUnirse($iduser, $estado) {
        ...
        $sql = "SELECT `sala_chat`.`idsala_chat`," .
                ...
                . "AND ( estado ='ACTIVO' or estado ='INVITADO' ) );";
//echo $sql;
        $resp = mysqli_query($conn, $sql);
        return $resp;
    }
    ...
    function buscarSalasChatPorIdUsuario($idUser, $estado) {
        ...
        $sql = "SELECT SC.`idsala_chat`," .
                ...
                "SC.`estado`," .
                "SC.`idadministrador`," .
                "(SELECT nombre FROM usuario WHERE idusuario = SC.`idadministrador`) as nombAdmin," .
...
//        echo $sql;
    ...
    function buscarSalasChatPorIdSalaChat($idSalaChat, $estado) {
        ...
        $sql = "SELECT SC.`idsala_chat`," .
                ...
                "SC.`estado`," .
                "SC.`idadministrador`," .
                "(SELECT COUNT(`idsala_chat`) as participantes FROM participante_sala WHERE `idsala_chat` = SC.`idsala_chat` AND estado ='ACTIVO') as participante, " .
                "( SELECT usuario.nombre FROM usuario WHERE usuario.idusuario = SC.idadministrador) as administrador" .
                " FROM `sala_chat` SC where `idsala_chat` IN (SELECT `participante_sala`.`idsala_chat` "
                . "FROM `participante_sala` "
