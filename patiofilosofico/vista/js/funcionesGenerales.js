...
//    inicioLoader();
//    oper = oper.replace(' ', '+');
    var paueteDatos = new FormData();
    paueteDatos.append('oper', oper);
    console.log(paueteDatos);
    $.ajax({
        url: urlClase,
        type: 'POST', // Siempre que se envíen ficheros, por POST, no por GET.
        contentType: false,
        data: paueteDatos, // Al atributo data se le asigna el objeto FormData.
        processData: false,
        cache: false,
...
function ejecutarFuncionElm(urlClase, oper, id, idContRespuesta, remplazarContenido) {
//    mostrarLoaderOndaDeCubos('Procesando...');
//    inicioLoader();
//    oper = oper.replace(' ', '+');
    var paueteDatos = new FormData();
    paueteDatos.append('oper', oper);
    paueteDatos.append('id', id);
    console.log(paueteDatos);
    $.ajax({
        url: urlClase,
        type: 'POST', // Siempre que se envíen ficheros, por POST, no por GET.
...
function cargarPagina2(uri, idContRespuesta, remplazo) {
//    mostrarLoaderOndaDeCubos('Procesando...');
//    cargarLoader();
...
function previsualizarImageenDiv(inpuFile, contImagen) {
    var file = inpuFile.files[0],
            imageType = /image.*/;
console.log("iniciando Previsualizacion imagen");
    if (!file.type.match(imageType))
        return;
